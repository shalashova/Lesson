			...
			<?php
				...
				foreach ($filenames as $file) {
					if($file[0] != "." ) {
						...
						$ss = stat($path . $file);
						$size = fSize($ss['size']);
						$lang_short = get_site_lang(true);
						if ($lang_short == 'ru') {
							echo '<tr>
								<td><i class="fa-solid fa-file-zipper fa-xl" style="color: #222831;"></i> &nbsp;&nbsp;<a title="'.i18n_r('DOWNLOAD').' '. str_replace(array("January","February","March","April","May","June","July","August","September", "October","November","December"),array("января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"),$name) .'" href="download.php?file='. $path . $file .'&amp;nonce='.get_nonce("archive", "download.php").'">'.str_replace(array("January","February","March","April","May","June","July","August","September", "October","November","December"),array("января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"),$name).'</a></td>
						...
						echo '<tr>
								<td><i class="fa-solid fa-file-zipper fa-xl" style="color: #222831;"></i> &nbsp;&nbsp;<a title="'.i18n_r('DOWNLOAD').' '. $name .'" href="download.php?file='. $path . $file .'&amp;nonce='.get_nonce("archive", "download.php").'">'.$name .'</a></td>
								<td style="width:70px;text-align:right;" ><span>'.$size.'</span></td>
								<td class="delete" ><a class="delconfirm" title="'.i18n_r('DELETE_ARCHIVE').': '. $name .'?" href="deletefile.php?zip='. $file .'&amp;nonce='.get_nonce("delete", "deletefile.php").'">&times;</a></td>
							  </tr>';
						}
